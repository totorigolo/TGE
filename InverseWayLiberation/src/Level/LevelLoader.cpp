...
bool LevelLoader::ProcessWorld() {
    ...
    if (world) {
        ...
        Dialog::Information(L"Les monde n'a pas été défini dans le niveau.\nLes valeurs par défaut ont été utilisées.");
    }
...
bool LevelLoader::ProcessTextures() {
    ...
    myCheckError(textures.ToElement(),
    ...
    while (body) {
        ...
        if (body->Attribute("type")) {
            ...
            if (type == "static")
                ...
                Dialog::Error(L"PolyBody type inconnu (" + type + L") !\nStatique par défaut.");
        } else
            Dialog::Error(L"Poly type introuvable !\nStatique par défaut.");
        ...
        if (polyBodiesDone) {
            ...
            if (pct == "chain")
                ...
                Dialog::Error(L"Poly ChainType inconnu (" + sf::String(pct) + L") !\nChaine par défaut.");
        }
        ...
        if (!mResourceManager.TextureExists(texture)) {
            Dialog::Error(L"Texture \"" + texture + L"\" inconnue. Body ignoré !");
        ...
        if (bb) {
            ...
            if (!name.isEmpty()) {
                ...
                if (r) Dialog::Error(L"Le nom " + name + L" apparait plusieurs fois !");
            }
            ...
            if (osp && bullet)
                Dialog::Error(L"Impossible d'avoir osp et bullet en même temps !");
            else if (osp)
...
bool LevelLoader::ProcessBasicBodies() {
    ...
    while (body) {
        ...
        // Récupère le type
        if (body->Attribute("type")) {
            sf::String stype = body->Attribute("type");
            if (stype == "static")
                ...
                Dialog::Error(L"BasicBody type inconnu (" + stype + ") !");
        }
        ...
        if (forme == "box") {
            ...
            bb->CreateBox(posRot, type, mResourceManager.GetTexture(texture), density, friction, restitution,
                          groupIndex, categoryBits, maskBits);
        } else if (forme == "circle") {
            bb = new BasicBody(layer);
            bb->CreateCircle(posRot, type, mResourceManager.GetTexture(texture), density, friction, restitution,
                             groupIndex, categoryBits, maskBits);
        } else
            Dialog::Error(L"BasicBody forme inconnu (" + forme + L") !");
...
bool LevelLoader::ProcessEntities() {
    ...
    myCheckError(entities.ToElement(),
    ...
    while (entity) {
        ...
        if (type == "ragdoll") {
            ...
            if (!name.isEmpty())
                Dialog::Error(L"Impossible de nommer un Ragdoll (" + name + L").");
        } else if (type == "player") {
            ...
            Dialog::Error(L"Entity type inconnu (" + type + L") !");
        }
...
bool LevelLoader::ProcessJoints() {
    ...
    while (joint) {
        ...
        if (e1 == nullptr) {
            Dialog::Error(L"L'entity #" + sf::String(Parser::uintToString(IDb1)) + L" est introuvable !");
        ...
        if (e2 == nullptr) {
            Dialog::Error(L"L'entity #" + sf::String(Parser::uintToString(IDb2)) + L" est introuvable !");
        ...
        std::function<b2Body * (Entity * )> getBody = [](Entity *e) {
            if (e->GetType() == EntityType::BaseBody || e->GetType() == EntityType::BasicBody ||
                e->GetType() == EntityType::PolyBody || e->GetType() == EntityType::PolyChain) {
        ...
        b2 = getBody(e2);
        if (!b1) {
            Dialog::Error(L"L'entity #" + sf::String(Parser::uintToString(IDb1)) +
                          L" n'est pas utilisable avec des Joints !");
        ...
        if (!b2) {
            Dialog::Error(L"L'entity #" + sf::String(Parser::uintToString(IDb2)) +
                          L" n'est pas utilisable avec des Joints !");
        ...
        if (type == "gear") {
            ...
            if (mJointIDMap.find(IDj1) == mJointIDMap.end()) {
                Dialog::Error(L"Le joint #" + sf::String(Parser::uintToString(IDj1)) + L" est introuvable !\n"
            ...
            if (mJointIDMap.find(IDj2) == mJointIDMap.end()) {
                Dialog::Error(L"Le joint #" + sf::String(Parser::uintToString(IDj2)) + L" est introuvable !\n"
                        "Il doit être construit avant.");
        ...
        if (j != nullptr && hasID) {
            ...
            if (mJointIDMap.find(id) != mJointIDMap.end())
                Dialog::Error(L"L'ID du Joint " + sf::String(Parser::intToString(id)) + L" n'est pas unique !", false);
...
bool LevelLoader::ProcessActions() {
    ...
    while (action) {
        ...
        if (type == "file") {
            ...
            // Crée l'action
            ScriptAction *a = new ScriptAction(name, file, once);
            ...
            // Crée l'action
            ScriptAction *a = new ScriptAction(name, file, function, once);
            mTriggersManager.AddAction(a);
        } else {
            Dialog::Error(L"Action type inconnu (" + sf::String(type) + L") !");
        }
...
bool LevelLoader::ProcessLights() {
    ...
    while (light) {
        ...
        if (type == "point") {
            ...
            Dialog::Error(L"Light type inconnu (" + sf::String(type) + L") !");
        }
