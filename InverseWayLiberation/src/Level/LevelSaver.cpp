...
LevelSaver::LevelSaver(const std::string &path, bool check)
    ...
    if (mIsValid)
        ...
        Dialog::Error(L"Sauvegarde impossible :\nle fichier est invalide.");
}
...
bool LevelSaver::ProcessBodies() {
    ...
    for (auto it = mEntityManager.GetEntities().begin(); it != mEntityManager.GetEntities().end(); ++it) {
        ...
        if ((*it)->GetType() == EntityType::BasicBody || (*it)->GetType() == EntityType::PolyBody ||
            (*it)->GetType() == EntityType::PolyChain) {
            BaseBody * bb = ((BaseBody *) *it);

            // Vérifie que l'Entity est créée
            if (!bb->IsCreated()) {
                Dialog::Error(L"Erreur lors de la sauvegarde :\nBaseBody non créé\nBaseBody ignoré.");
                continue;
            }
...
bool LevelSaver::ProcessJoints() {
    ...
    for (auto it = mPhysicManager.GetJointList().begin(); it != mPhysicManager.GetJointList().end(); ++it) {
        ...
        myAssert(j->GetBodyA()->GetUserData(),
                 "Le Joint #" + Parser::intToString(j->GetID()) + " a un b2BodyA sans Entity.");
        myAssert(j->GetBodyB()->GetUserData(),
                 "Le Joint #" + Parser::intToString(j->GetID()) + " a un b2BodyB sans Entity.");
        ...
        if (j->GetType() == JointType::DistanceJoint) {
            ...
            if (mJointIDMap.find(gj->GetJoint1()) == mJointIDMap.end()) {
                ...
                myAssert(j1,
                         "Le joint1 du GearJoint #" + Parser::intToString(gj->GetID()) + " n'est pas un Joint valide.");
            ...
            if (mJointIDMap.find(gj->GetJoint2()) == mJointIDMap.end()) {
                ...
                myAssert(j2,
                         "Le joint2 du GearJoint #" + Parser::intToString(gj->GetID()) + " n'est pas un Joint valide.");
        ...
        if (balise) {
            ...
            if (j->GetMaxForceType() == ForceType::Vector && maxVecForce.LengthSquared() != 0)
                balise->SetAttribute("maxVecForce", maxForce);
