...
Hum::Hum(int layer)
    ...
    // Bulle de texte
    ResourceManager::GetInstance().LoadFont("hum-text", "data/hums.otf");
    mBallonText.setFont(*ResourceManager::GetInstance().GetFont("hum-text"));
...
void Hum::PreUpdate() {
    ...
    if (mBody && mBodyIsCreated && mIsAlive) {
        ...
        mEyeRight.setPosition(
                mTrunk.getPosition() + sf::Vector2f(-(4.f + mEyeRight.getRadius() * 2.f), (float) -mEyeHeight));
        mPupilLeft.setPosition(mEyeLeft.getPosition() + sf::Vector2f(mEyeLeft.getRadius() - mPupilLeft.getRadius(),
                                                                     mEyeLeft.getRadius() - mPupilLeft.getRadius()));
        mPupilRight.setPosition(mEyeRight.getPosition() + sf::Vector2f(mEyeRight.getRadius() - mPupilRight.getRadius(),
                                                                       mEyeRight.getRadius() -
                                                                       mPupilRight.getRadius()));
...
void Hum::Update() {
    ...
    if (mBody && mBodyIsCreated && mIsAlive) {
        ...
        if (mSpeaking) {
            ...
            pos += sf::Vector2f(-mBallonText.getGlobalBounds().width / 2.f,
                                -mBallonText.getGlobalBounds().height * 2.f);
