...
void BasicBodyCreationWindow::Add(b2Vec2 pos) {
    ...
    if (mShape[0]->IsActive()) // Box
        EntityFactory::CreateBox(posRot, type, mTexture->GetItem(mTexture->GetSelectedItem()).toAnsiString(),
                                 static_cast<int>(mLayer->GetValue()));
    else if (mShape[1]->IsActive()) // Circle
        EntityFactory::CreateCircle(posRot, type, mTexture->GetItem(mTexture->GetSelectedItem()).toAnsiString(),
                                    static_cast<int>(mLayer->GetValue()));
    else {
...
void BasicBodyCreationWindow::Fill() {
    ...
    // Signaux
    mToggleModeBtn->GetSignal(sfg::Button::OnLeftClick).Connect(
            std::bind(&BasicBodyCreationWindow::OnToggleMode, this));
