...
void PolyCreationWindow::Fill() {
    ...
    // Signaux
    mEnterEditModeBtn->GetSignal(sfg::Button::OnLeftClick).Connect(
            std::bind(&PolyCreationWindow::OnToggleEditMode, this));
...
void PolyCreationWindow::OnCreatePoly() {
    ...
    if (mType[0]->IsActive()) // Body
    {
        ...
        // Crée le Poly
        EntityFactory::CreatePolyBody(mPoints, type, mTexture->GetItem(mTexture->GetSelectedItem()).toAnsiString(),
                                      static_cast<int>(mLayer->GetValue()));
    } else if (mType[1]->IsActive()) // Chain
    {
        // Crée la Chaine
        auto type = PolyChain::Type::Chain;
        EntityFactory::CreatePolyChain(mPoints, type, mTexture->GetItem(mTexture->GetSelectedItem()).toAnsiString(),
                                       static_cast<int>(mLayer->GetValue()));
