...
void ColFilteringWindow::Fill() {
    ...
    mGroupIndexLabel = sfg::Label::Create(L"GroupIndex : ");
    mGroupIndex = sfg::SpinButton::Create(-32768.f, 32767.f, 1.f);
    mBitsTable = sfg::Table::Create();
    mCatBitsLabel = sfg::Label::Create(L"Category Bits :");
    mMaskBitsLabel = sfg::Label::Create(L"Mask Bits :");
    mBitsTable->Attach(mCatBitsLabel, sf::Rect<sf::Uint32>(1, 1, 16, 1));
    mBitsTable->Attach(mMaskBitsLabel, sf::Rect<sf::Uint32>(1, 3, 16, 1));
    mColFilteringApply = sfg::Button::Create(L"Appliquer");
    mColFilteringApply->GetSignal(sfg::Button::OnLeftClick).Connect(
            std::bind(&ColFilteringWindow::OnColFilteringApply, this));
    mColFilteringRefresh = sfg::Button::Create(L"Actualiser");
    mColFilteringRefresh->GetSignal(sfg::Button::OnLeftClick).Connect(std::bind(&ColFilteringWindow::OnRefresh, this));
    mColFilteringClose = sfg::Button::Create(L"Fermer");
