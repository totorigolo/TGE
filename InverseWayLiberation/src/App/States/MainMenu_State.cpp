...
State *MainMenu_State::Run(App *app) {
    ...
    if (!fondTex.loadFromFile("tex/main_menu_bckg.jpg"))
        Dialog::Error(L"Impossible de charger l'image de fond du menu.");
    ...
    if (!arrowTex.loadFromFile("tex/menu_arrow.png"))
        Dialog::Error(L"Impossible de charger l'image de flèche du menu.");
    ...
    while (window->isOpen() && !quit) {
        ...
        if (mInputManager.HasResized()) {
            view.reset(sf::FloatRect(0, 0, static_cast<float>(fondTex.getSize().x),
                                     static_cast<float>(fondTex.getSize().y)));
        }
        // Flèche haut
